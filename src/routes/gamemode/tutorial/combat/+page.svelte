    ...
    {#if cardsDrawn}
        <div class="absolute w-full h-full bg-black/70 top-0 z-10">
            <div class="flex flex-grow flex-col sm:flex-row items-center justify-evenly h-full">
                <div on:click={(e) => pickedCard(0)} class="cursor-pointer" in:fly|local="{{ y: 200, x: 800, duration: 1000 }}">
                    <CursedCard type="basic" />
                </div>
                <div on:click={(e) => pickedCard(1)} class="cursor-pointer" in:fly|local="{{ y: 200, x: 600, delay: 500, duration: 1000 }}">
                    <CursedCard type="basic" />
                </div>
                <div on:click={(e) => pickedCard(2)} class="cursor-pointer" in:fly|local="{{ y: 200, x: 100, delay: 1000, duration: 1000 }}">
                    <CursedCard type="basic" />
                </div>
            </div>
        </div>
